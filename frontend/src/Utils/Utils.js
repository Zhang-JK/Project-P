...
export const createFeedback = (value, callback, failedCallback) => {
    if (!sendingFeedback) {
        sendingFeedback = true;
        postRequest("createFeedback", {msg: value}).then(result => {
            if (result.code === 200) {
                alert("create meg success!")
                if (!(callback===undefined)){
                    callback()
...
export const editFeedback = (fbId, value, callback, failedCallback) => {
    if (!editingFeedback) {
        editingFeedback = true;
        postRequest("editFeedback", {msg: value, fbId: fbId}).then(result => {
            if (result.code === 200) {
